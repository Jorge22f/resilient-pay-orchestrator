...
describe("Resilient Pay Orchestrator Base Integration Suite", () => {
  ...
  before(async () => {
    const isDockerNetwork =
      process.env.GITHUB_ACTIONS === "true" || process.env.IS_DOCKER === "true";

    const dbHost = isDockerNetwork
      ? process.env.DB_HOST || "localhost"
      : "127.0.0.1";
    const redisHost = isDockerNetwork
    ...
    const localKnexConfig = {
      ...
      connection: {
        ...
        database: process.env.DB_NAME || "payment_orchestrator_test",
        port: 5432,
      },
    };
